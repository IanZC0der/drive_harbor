...
/**
 * distributed lock universal point out
 */
@Component
@Aspect
@Slf4j
public class LockAspect implements ApplicationContextAware {
    ...
    public Object aroundLock(ProceedingJoinPoint proceedingJoinPoint) {
        ...
        if (Objects.isNull(lock)) {
            log.error("lock aspect acquire lock fail.");
            throw new driveHarborFrameworkException("aroundLock acquire lock fail");
        }
        ...
        try {
            ...
            throw new driveHarborFrameworkException("aroundLock tryLock fail.");
        } catch (Throwable e) {
            log.error("lock aspect tryLock exception.", e);
            throw new driveHarborFrameworkException("aroundLock tryLock fail.");
        } finally {
