...
public class FileController {
    ...
    @ApiOperation(
            value = "file search",
            notes = "This interface provides the functionality of file search",
            consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE,
            produces = MediaType.APPLICATION_JSON_UTF8_VALUE
    )
    ...
    public R<List<FileSearchResultVO>> search(@Validated FileSearchPO fileSearchPO) {
        ...
        if (StringUtils.isNotBlank(fileTypes) && !Objects.equals(FileConstants.ALL_FILE_TYPE, fileTypes)) {
            List<Integer> fileTypeArray = Splitter.on(driveHarborConstants.COMMON_SEPARATOR).splitToList(fileTypes).stream().map(Integer::valueOf).collect(Collectors.toList());
            context.setFileTypeArray(fileTypeArray);
        }
    ...
    @ApiOperation(
            value = "query bread crumbs list",
            notes = "This interface provides the functionality of querying crumbs list",
            consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE,
            produces = MediaType.APPLICATION_JSON_UTF8_VALUE
    )
    @GetMapping("file/breadcrumbs")
    public R<List<BreadcrumbVO>> getBreadcrumbs(@NotBlank(message = "fileId cannot be null.") @RequestParam(value = "fileId", required = false) String fileId) {
